/* ═══════════════════════════════════════════════════════════
   ...
   Camera: CloudCompare-style 
   Features:
     - Hover cluster in right panel → highlight in main view
     - Click cluster → instant appear/disappear (no loading)
     - Double-click → mark → CSV with original XYZ
   ═══════════════════════════════════════════════════════════ */
(() => {
"use strict";
...
function onRowDoubleClick(event, ri) {
  ...
  if (intersects.length > 0) {
    ...
    const clickData = {
      ...
      timestamp: Date.now(),
      cloudName: cloudData ? cloudData.name : 'unknown',
      clusterId: trueClusterId, viewType: `linearized_row_${ri.idx}`,
...
function addClickLogEntry(clickData) {
  ...
  const origStr = clickData.originalX !== '' && clickData.originalX !== undefined
    ? `Orig(${Number(clickData.originalX).toFixed(3)}, ${Number(clickData.originalY).toFixed(3)}, ${Number(clickData.originalZ).toFixed(3)})`
    : '';
  const div = document.createElement('div');
  div.className = 'click-log-entry';
  div.innerHTML = `<span class="cle-id">#${clickData.id}</span> Cluster ${clickData.clusterId} · Pt ${clickData.pointIndex} <span class="cle-coord">${origStr}</span>`;
...
function removeClickLogEntry(clickId) {
  const body = document.getElementById("clickLogBody");
  if (!body) return;
  const entries = body.querySelectorAll('.click-log-entry');
  entries.forEach(el => {
    if (el.querySelector('.cle-id')?.textContent === `#${clickId}`) el.remove();
  });
  if (!body.querySelector('.click-log-entry')) {
    body.innerHTML = '<div class="empty">Double-click points to log them here.</div>';
  }
}

function clearClickLog() {
  const body = document.getElementById("clickLogBody");
  if (body) body.innerHTML = '<div class="empty">Double-click points to log them here.</div>';
}
...
function loadRawCloud(data) {
  ...
  status(`Loaded: ${data.name} — ${data.meta.num_points.toLocaleString()} pts. Click "Cluster & Linearize" to begin.`);
...
function updateDBTree() {
  ...
  if (isRawMode) {
    tree.innerHTML = `<div class="dbi sel"><span class="dbi-icon">☁️</span><span class="dbi-name" title="${cloudData.name}">${cloudData.name}</span><span class="dbi-badge">raw</span><span class="dbi-del" onclick="event.stopPropagation();window._clearAll()">✕</span></div>
    ...
    tree.innerHTML = `<div class="dbi sel"><span class="dbi-icon">☁️</span><span class="dbi-name" title="${cloudData.name}">${cloudData.name}</span><span class="dbi-badge">${m.n_clusters || 0} cls</span><span class="dbi-del" onclick="event.stopPropagation();window._clearAll()">✕</span></div>
...
window._clearAll = () => { clearScene(); clickedPoints = []; clickIdCounter = 0; updateClickCount(); updateDBTree(); showEmptyProps(); showEmptyClusterProps(); document.getElementById("btnRecluster").textContent = "Cluster & Linearize"; document.getElementById("btnApplyOverlap").disabled = true; status("Cleared"); };
...
function showMainProps() {
  ...
  if (m.annotation) {
    html += `<div class="ps"><div class="pst">Annotation</div><div class="pr"><span class="pk">File</span><span class="pv">${m.annotation}</span></div><div class="pr"><span class="pk">Labels</span><span class="pv">${(m.annotation_labels || 0).toLocaleString()}</span></div><div class="pr"><span class="pk">Classes</span><span class="pv">${m.annotation_classes || 0}</span></div></div>`;
  }
  body.innerHTML = html;
}
...
function setupUpload() {
  ...
  fi.addEventListener("change", () => { if (fi.files.length > 0) doUpload(fi.files[0]); });
  zone.addEventListener("dragover", e => { e.preventDefault(); zone.classList.add("dragover"); });
  zone.addEventListener("dragleave", () => zone.classList.remove("dragover"));
  zone.addEventListener("drop", e => { e.preventDefault(); zone.classList.remove("dragover"); if (e.dataTransfer.files.length > 0) doUpload(e.dataTransfer.files[0]); });
  ...
  annFi.addEventListener("change", () => {
    if (annFi.files.length > 0) { annName.textContent = annFi.files[0].name; clearAnn.style.display = "inline"; }
    else { annName.textContent = "No file selected"; clearAnn.style.display = "none"; }
  });
  clearAnn.addEventListener("click", () => { annFi.value = ""; annName.textContent = "No file selected"; clearAnn.style.display = "none"; });
...
function updateColorScaleBar(mode) {
  const bar = document.getElementById("colorScaleBar");
  const title = document.getElementById("colorScaleTitle");
  const gradient = document.getElementById("colorScaleGradient");
  const labels = document.getElementById("colorScaleLabels");
  ...
  if (mode === 'normal') {
    // title.textContent = "Normal Direction (|Nx| → R, |Ny| → G, |Nz| → B)";
    gradient.style.background = "linear-gradient(to right, rgb(255,0,0), rgb(255,255,0), rgb(0,255,0), rgb(0,255,255), rgb(0,0,255))";
    labels.innerHTML = "<span>X (Red)</span><span>Y (Green)</span><span>Z (Blue)</span>";
  } else if (mode === 'curvature') {
    // title.textContent = "Curvature (λ_min / Σλ)";
    gradient.style.background = "linear-gradient(to right, rgb(0,255,0), rgb(0,128,128), rgb(0,0,255))";
    labels.innerHTML = "<span>Low (flat)</span><span>Medium</span><span>High (curved)</span>";
  }
}
...
function onKeyDown(e) {
  ...
  switch (e.key) {
    ...
    case "+": case "=": setPtSize(Math.min(10, parseFloat(document.getElementById("ptSlider").value) + 0.5)); break;
    case "-": case "_": setPtSize(Math.max(0.5, parseFloat(document.getElementById("ptSlider").value) - 0.5)); break;
