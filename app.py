...
def build_response(pts, cols, labels, cluster_colors, n_cls, overlap, meta=None, store=None):
    ...
    cp, cc, ct, cd, cls_orig_idx = subsample(pts_c, cluster_col_arr, 2_000_000, original_indices)
...
def upload_raw():
    ...
    try:
        ...
        # Handle optional annotation / ground truth file
        annotation_info = None
        if 'annotation' in request.files:
            ann_file = request.files['annotation']
                ...
                try:
                    ...
                    with open(ann_path) as af:
                        for line in af:
                            ...
                            try: ann_labels.append(int(float(line.replace(',', ' ').split()[0])))
                            except: continue
                    unique_classes = sorted(set(ann_labels))
                    annotation_info = {
                        'name': ann_fn,
                        'labels': ann_labels,
                        'unique_classes': unique_classes,
                        'num_labels': len(ann_labels),
                        'num_classes': len(unique_classes),
                    }
                    meta['annotation'] = ann_fn
                    meta['annotation_classes'] = len(unique_classes)
                    meta['annotation_labels'] = len(ann_labels)
                except Exception as ae:
                    print(f"Annotation parse warning: {ae}")

        # Store raw data (no clustering yet)
        cloud_store[uid] = {
        ...
        # Center and subsample for display (white for display, originals in store)
        pts_c = (pts - pts.mean(0)).astype(np.float32)
        ...
        if annotation_info:
            result['annotation'] = {
                'name': annotation_info['name'],
                'num_labels': annotation_info['num_labels'],
                'num_classes': annotation_info['num_classes'],
                'unique_classes': annotation_info['unique_classes'],
            }
